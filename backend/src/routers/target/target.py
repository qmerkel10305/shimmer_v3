...
from routers.target.models import RequestTarget
...
async def put_target(target_id: int, target: RequestTarget, db: Session = Depends(get_db)):
    tu = UpdateTarget(**target.dict())
    return update_target(tu, db)

@router.post('<int:target_id>/', response_model = Target)
...
@router.delete('/<int:target_id>', response_model = Target)
