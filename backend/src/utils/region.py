...
def get_thumbnail(flight_folder:str, target_region_id:int) -> Path:
...
def create_thumbnail(flight_folder:str, flight_id: int, target_id: int, target_region_id:int, db: Session) -> Path:
    tr = db.query(models.TargetRegion).filter(models.TargetRegion.flight_id == flight_id).filter(models.TargetRegion.target_id == target_id).filter(models.TargetRegion.target_region_id == target_region_id).one()
